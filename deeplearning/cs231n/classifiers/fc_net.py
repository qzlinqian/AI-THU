...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i, hid_dim in enumerate(hidden_dims):
            self.params['W%d'%(i+1)] = weight_scale * np.random.randn(layer_input_dim, hid_dim)
            self.params['b%d'%(i+1)] = weight_scale * np.zeros(hid_dim)
            if normalization is "batchnorm":
                self.params['gamma%d'%(i+1)] = np.ones(hid_dim)
                self.params['beta%d'%(i+1)] = np.zeros(hid_dim)
            layer_input_dim = hid_dim
    ...
    def loss(self, X, y=None):
        ...
        for layer in range(self.num_layers-1):
            if self.normalization == "batchnorm":
                layer_input, bc_cache[layer] = affine_bn_relu_forward(layer_input, 
                ...
                layer_input, bc_cache[layer] = affine_relu_forward(layer_input, 
                                                        self.params['W%d'%(layer+1)], 
                                                        self.params['b%d'%(layer+1)])
        ...
        ar_out, bc_cache[self.num_layers] = affine_forward(layer_input, 
        ...
        dx , dw , db = affine_backward(dhout , bc_cache[self.num_layers])
        ...
        for idx in range(self.num_layers-1):
            ...
            if self.normalization == "batchnorm":
                dx, dw, db, dgamma, dbeta = affine_bn_relu_backward(dhout, bc_cache[layer])
            else:
                dx, dw, db = affine_relu_backward(dhout, bc_cache[layer])
